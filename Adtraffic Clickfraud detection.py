...
##############################################################
# Open a list that will later be used to append all predictors
##############################################################
predictors=[]
...
def DO(frm,to,fileno):
    ...
    train_df = pd.read_csv("/dev/train.csv", parse_dates=['click_time'], skiprows=range(1,frm), nrows=to-frm, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])

    print('loading test data...')
    if debug:
        test_df = pd.read_csv("/dev/test_supplement.csv", nrows=100000, parse_dates=['click_time'], dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'click_id'])
    else:
        test_df = pd.read_csv("/dev/test_supplement.csv", parse_dates=['click_time'], dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'click_id'])
...
###################
    train_df = do_count(train_df, ['ip']); gc.collect()
    train_df = do_count(train_df, ['os', 'device']); gc.collect()
    train_df = do_count(train_df, ['os', 'app', 'channel']); gc.collect()

    train_df = do_count(train_df, ['ip', 'device']); gc.collect()
    train_df = do_count(train_df, ['app', 'channel']); gc.collect()

    train_df = do_count(train_df, ['ip', 'os', 'hour']); gc.collect()
    train_df = do_count(train_df, ['ip', 'app', 'hour']); gc.collect()
    train_df = do_count(train_df, ['ip', 'device', 'hour']); gc.collect()
    train_df = do_count(train_df, ['ip', 'app', 'os']); gc.collect()
    train_df = do_count(train_df, ['hour', 'app']); gc.collect()
    train_df = do_count(train_df, ['hour', 'channel']); gc.collect()
